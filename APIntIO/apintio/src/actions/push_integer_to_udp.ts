import { action, SendToPluginEvent,KeyDownEvent, KeyUpEvent, SingletonAction, WillAppearEvent } from "@elgato/streamdeck";
 ...
 */
@action({ UUID: "be.elab.apintio.pushintudp" })
export class PushIntegerThroughUDP extends SingletonAction<PushIntUDPSettings> {
	...
	override onWillAppear(ev: WillAppearEvent<PushIntUDPSettings>): void | Promise<void> {
		return ev.action.setTitle(`${ev.payload.settings.integerIndex ?? 0}`);
	}
	...
	override async onKeyDown(ev: KeyDownEvent<PushIntUDPSettings>): Promise<void> {
		...
		let text =
        ( ev.payload.settings.integerIndex??0) + "\n" +
         (ev.payload.settings.integerValueDown??0) + "\n" + 
         (ev.payload.settings.ipAddress??'') + "\n" + 
         (ev.payload.settings.ipPort??'');
        console.log(text);
...
//            ev.action.showOk();
	...
	override async onKeyUp(ev: KeyUpEvent<PushIntUDPSettings>): Promise<void> {
		...
		let text =
        ( ev.payload.settings.integerIndex??0) + "\n" +
         (ev.payload.settings.integerValueUp??0) + "\n" + 
         (ev.payload.settings.ipAddress??'') + "\n" + 
         (ev.payload.settings.ipPort??'');
        console.log(text);
        ev.action.setTitle(text);
